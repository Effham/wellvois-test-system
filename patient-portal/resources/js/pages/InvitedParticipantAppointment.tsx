...
interface InvitedParticipantAppointmentProps {
  roomId: string;
...
export default function InvitedParticipantAppointment({
  roomId,
  antMediaUrl,
  tenant,
  participant,
  appointmentId,
  appearanceSettings,
}: InvitedParticipantAppointmentProps) {
  ...
  // Build iframe URL with participant name parameters
  const buildIframeUrl = () => {
    ...
    if (participant?.name) {
      const participantName = toTitleCase(participant.name);
      queryParams.push(`name=${encodeURIComponent(participantName)}`);
      queryParams.push(`participant=yes`);
    }
      ...
      <div className="h-screen w-screen flex flex-col bg-white text-gray-900">
        ...
        <header className="bg-white/95 backdrop-blur-sm border-b border-gray-200 flex-shrink-0">
          ...
          <div className="mx-auto max-w-7xl px-6 py-3 flex items-center justify-between gap-4">
            ...
            {/* Participant info and session status */}
            <div className="flex items-center gap-4">
              ...
              {/* Session status */}
              <div className="flex items-center gap-2 rounded-full border px-3 py-1 text-xs text-gray-600 bg-white/70">
