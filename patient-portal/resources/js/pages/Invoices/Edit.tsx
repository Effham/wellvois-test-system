...
function EditInvoice({ invoice, customer }: Props) {
  const { props }: any = usePage();
  const [customerType, setCustomerType] = useState<'patient' | 'practitioner'>(
    customer?.type === 'practitioner' ? 'practitioner' : 'patient'
  );
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(
    customer ? {
      id: customer.wallet_id.toString(),
      name: customer.name,
      type: customer.type,
      type_id: customer.id,
  ...
  // Load existing line items or default
  const existingLines = invoice.meta?.lines?.map(line => ({
    description: line.desc,
    quantity: line.qty,
    unit_price: parseFloat(String(line.unit_price)),
    tax_rate: parseFloat(String(line.tax_rate)),
  })) || [{
    description: '',
    quantity: 1,
    unit_price: 0,
  ...
  const { data, setData, processing, errors, reset } = useForm({
    customer: customer?.wallet_id.toString() || '',
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const payload = {
      customer_wallet_id: parseInt(data.customer),
    ...
    // Use router.put instead of put from useForm to send custom payload
        ...
        <Card className="shadow-none border-none">
          <CardContent className="space-y-6 p-3 sm:p-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl sm:text-2xl font-bold">Edit Invoice #{invoice.id}</h2>
            </div>
...
export default withAppLayout(EditInvoice, {
  breadcrumbs: [
