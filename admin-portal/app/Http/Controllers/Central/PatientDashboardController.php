...
class PatientDashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ensure user has Patient role (additional safety check)
        if (! $user || ! $user->hasRole('Patient')) {
            ...
            return back()->with('error', 'Access denied. This dashboard is only for patients.');
        }
    ...
    /**
     * Get dashboard data for central context (aggregates from all patient's tenants)
     */
    public function getDashboardData(Request $request)
    {
        try {
            ...
            // Get all tenants this patient has access to
            $patientTenants = \DB::connection('central')->table('tenant_patients')
                ->where('patient_id', $centralPatient->id)
                ->where('invitation_status', 'ACCEPTED')
                ->get();

            if ($patientTenants->isEmpty()) {
                // Patient has no tenants yet, return empty data
            ...
            foreach ($patientTenants as $tenantPatient) {
                ...
                // Run queries in tenant context
                tenancy()->initialize($tenant);

                try {
                    // Get appointments from this tenant
                    $appointments = \App\Models\Tenant\Appointment::where('patient_id', $centralPatient->id)
                    ...
                    // Get medications from this tenant
                    $medications = \App\Models\Tenant\EncounterPrescription::whereHas('encounter.appointment', function ($query) use ($centralPatient) {
                        $query->where('patient_id', $centralPatient->id);
                    })
                        ->with(['encounter.appointment'])
                        ->get()
                        ->map(function ($prescription) use ($tenant) {
                            ...
                            return [
                                ...
                                'nextRefill' => now()->addDays(30)->format('M j, Y'),
                                'tenant_id' => $tenant->id,
                                'tenant_name' => $tenant->name,
                            ];
                        });

                    $allMedications = $allMedications->merge($medications);

                    // Get visits from this tenant
                    $visits = \App\Models\Tenant\Encounter::whereHas('appointment', function ($query) use ($centralPatient) {
                        $query->where('patient_id', $centralPatient->id);
                    })
                        ...
                        ->take(10)
                        ->get()
                        ->map(function ($encounter) use ($tenant) {
                    ...
                    $allVisits = $allVisits->merge($visits);

                    // Count visits this year from this tenant
                    ...
                    $totalVisitsThisYear += $visitsThisYear;
                } catch (\Exception $e) {
                    \Log::error('Error fetching data from tenant: '.$tenant->id, ['error' => $e->getMessage()]);
                } finally {
                    tenancy()->end();
                }
            }
            ...
            $sortedAppointments = $allAppointments->sortBy('date')->take(5)->values();
            $uniqueMedications = $allMedications->unique('name')->take(10)->values();
            $sortedVisits = $allVisits->sortByDesc('date')->take(5)->values();

            $quickStats = [
                'nextAppointment' => $sortedAppointments->first()['date'] ?? null,
                'activeMedications' => $uniqueMedications->count(),
                'visitsThisYear' => $totalVisitsThisYear,
            ];

            return response()->json([
                'upcomingAppointments' => $sortedAppointments,
                'currentMedications' => $uniqueMedications,
                'recentVisits' => $sortedVisits,
                'quickStats' => $quickStats,
                'patientInfo' => array_merge(
                    (new PatientDashboardResource($centralPatient))->resolve(),
                    ['tenant_count' => $patientTenants->count()]
                ),
            ]);
